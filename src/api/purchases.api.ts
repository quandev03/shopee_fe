...
export const purchasesApi = {
  ...
  addToCart: (body: ProductCart) => {
    return api.post<ResponseSuccessType<Purchases>>(
      `${URL}/add-product-in-cart?productId=${body.product_id}&quantity=${body.buy_count}`
    );
  },
  updatePurchases: (body: ProductCart) => {
    return api.put<ResponseSuccessType<Purchases>>(
      `${URL}/update-purchase`,
      body
    );
  },

...
export default function Cart() {
  ...
  const purchaseChosenFromProductDetail = (
  ...
  const productInCartData = useMemo(() => {
    return (
      purchasesData?.data.map((cart: any) => ({
        _id: cart.id,
  ...
  const isCheckAllPurchase = useMemo(
    () => extendsPurchases.every((purchase) => purchase.checked),
    [extendsPurchases]
  );
  const checkedPurchases = useMemo(
    () => extendsPurchases.filter((purchase) => purchase.checked),
    [extendsPurchases]
  );
  ...
  useEffect(() => {
    setExtendsPurchases((prev) => {
      ...
      return (
        productInCartData?.map((purchase: Purchases) => ({
          ...purchase,
          checked:
            purchaseChosenFromProductDetail === purchase._id ||
            Boolean(purchasesObject[purchase._id]?.checked),
          disabled: false,
        })) || []
      );
    });
                          ...
                          <div className="flex">
                            ...
                            >
                              <img
                                ...
                                alt={purchase.product.nameProduct}
                              />
                            </Link>
                            <Link
                              to={`${path.home}${generateNameId({
                            ...
                            onFocusOut={(value) =>
                              handleQuantity(
                                ...
                                value >= 1 &&
                                  value < purchase.product.quantity &&
                                  value !==
                                    (productInCartData as Purchases[])[index]
                                      .buy_count
                              )
                            }
                          />
                    ...
                    <div>
                      <div className="flex items-center gap-1">
                        <p className="text-[12px] sm:text-[16px]">
                          Tổng thanh toán ({checkedPurchasesCount}):{" "}
                        </p>

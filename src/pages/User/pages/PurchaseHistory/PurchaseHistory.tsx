...
export default function PurchaseHistory() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      purchasesApi.getPurchases({ status: status as PurchaseListStatus }),
    retry: 0,
  });
  ...
  return (
    <div className="relative">
      <Helmet>
        ...
        <meta
          name="description"
          content="Thông tin đơn mua của tôi trên shopee"
        />
      </Helmet>
            ...
            {tabArrayData.map((tab) => (
              <Link
                ...
                className={classNames(
                  "flex-1 border-b-[2px] bg-white py-4 text-center",
                  {
          ...
          {!isLoading &&
            (dataPurchases && dataPurchases.length ? (
              dataPurchases.map((purchase) => (
                <div
                  key={purchase._id}
                  className="mt-3 bg-white px-6 py-4 shadow-sm"
                >
                  <Link
